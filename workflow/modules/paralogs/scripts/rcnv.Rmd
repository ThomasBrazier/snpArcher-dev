...
# Save allele.info
write.table(A.info, file = paste0("../../../../results/", refGenome, "/paralogs/rCNV/", prefix, "_allele_info.csv"), quote = F, row.names = F, col.names = T, sep = "\t")
...
deviants = dupGet(A.info, test = c("z.05","chi.05"), plot = F, verbose = TRUE)
head(deviants)

table(deviants$dup.stat)

idx = which(deviants$dup.stat == "deviant")

deviantBED = data.frame(chrom = CNV$CHROM[idx],
                    chromStart = CNV$POS[idx] - 1,
                    chromEnd = CNV$POS[idx])
...
write.table(deviants, file = paste0("../../../../results/", refGenome, "/paralogs/rCNV/", prefix, "_deviant.tsv"), quote = F, row.names = F, col.names = T, sep = "\t")
write.table(deviantBED, paste0("../../../../results/", refGenome, "/paralogs/rCNV/", prefix, "_deviant.BED") , quote = F, row.names = F, col.names = F, sep = "\t")
...
CNV = cnv(A.info, test=c("z.05","chi.05"), filter = "kmeans", intersection = TRUE)

# see the difference between deviants and duplicates
table(deviants$dup.stat)
table(CNV$dup.stat)

idx = which(CNV$dup.stat == "cnv")

cnvBED = data.frame(chrom = CNV$CHROM[idx],
                    chromStart = CNV$POS[idx] - 1,
                    chromEnd = CNV$POS[idx])
...
write.table(cnvBED, paste0("../../../../results/", refGenome, "/paralogs/rCNV/", prefix, "_CNV.BED") , quote = F, row.names = F, col.names = F, sep = "\t")
...
scaffold.size = 10000
duplicates = dup.validate(CNV, window.size = 100, scaf.size = scaffold.size)

chrom = gsub("\\.[0-9]+$",  "", duplicates$CHROM)
pos = gsub("^[0-9A-Za-z]+\\.[0-9]+\\.",  "", duplicates$CHROM)
pos = as.numeric(pos)
end = pos * scaffold.size - 1
start = end - 9999

duplicates = data.frame(chrom = chrom,
                        start = start,
                        end = end,
                        dp.ratio = duplicates$dp.ratio,
                        CHROM.length = duplicates$CHROM.length,
                        no.duplicates = duplicates$no.duplicates,
                        no.singlets = duplicates$no.singlets)

head(duplicates)

write.table(CNV, paste0("../../../../results/", refGenome, "/paralogs/rCNV/", prefix, "_duplicates.tsv") , quote = F, row.names = F, col.names = T, sep = "\t")
```
